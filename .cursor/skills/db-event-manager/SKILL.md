...
| 데이터 | `STORE_BACKEND`: `json`→`data/` JSON; `rdb`→시스템 MSSQL(`DB_SYSTEM_*`, `persistence/`). **RDB→JSON 파일 동기는 불필요**(이관·백필은 JSON→메모리→저장 시 RDB 플러시). |
...
- **시스템 DB**: `db/systemDb.ts`는 **`STORE_BACKEND=rdb`** 시 도메인 영속 + 마이그레이션용 **MSSQL 전용**. 타깃 게임 DB 실행과 별개.
- **영속 스위치**: `STORE_BACKEND=json`(기본) \| `rdb`. **`json`**: 읽기·쓰기 원천은 `data/*.json`. **`rdb`**: 쓰기 원천은 시스템 DB 테이블; **디스크 JSON을 RDB와 맞출 의무 없음**. 부팅: 마이그레이션 후 `fnBootstrapPersistence()`로 RDB→메모리 하이드레이트(게이트·테이블 건수는 `persistence/rdb/*`). 인스턴스 페이로드는 `event_instances.str_payload`. **이관**: `json`으로 메모리 채운 뒤 `rdb`로 전환해 저장 API가 플러시하거나, 빈 RDB에서 운영하며 저장으로 적재. 롤백: `STORE_BACKEND=json` + `data/` 백업 복원 · `npm run backup:data`(backend).
...
```
backend/src/
  persistence/                            # STORE_BACKEND, RDB 하이드레이트·플러시(rdb/*)
...
- **코드**: `backend/src/data/` — 런타임은 항상 `arr*` 인메모리.
- **`STORE_BACKEND=json`**: `DATA_DIR`(미설정 시 `backend/data/`) JSON이 읽기·쓰기 원천.
- **`STORE_BACKEND=rdb`**: 시스템 MSSQL이 접속·RBAC·프로덕트·쿼리템플릿·인스턴스 등의 **쓰기 원천**; 부팅 시 조건에 따라 RDB→메모리 하이드레이트. **RDB 스냅샷을 JSON으로 보낼 필요 없음.**
- **항상 JSON 파일**: 활동 로그(`activity_logs.json`), UI 설정(`userUiPreferences.json`) — `STORE_BACKEND`와 무관.
- **`seed_test.json`**: 하이드레이트 이후 메모리 덮어씀. **`STORE_BACKEND=rdb`면 기본 무시** — 로컬 시드+RDB 반영 시 `SEED_TEST_WITH_RDB=1`(시드 후 접속·카탈로그·auth 플러시).
- 스키마: `docs/schema.sql` · `docs/schema_normalized.sql` · `docs/SCHEMA-DATA-REVIEW.md`. 롤백: `STORE_BACKEND=json` + `npm run backup:data` 복원.
