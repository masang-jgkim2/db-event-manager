...
const ARR_SEED: IRolePermissionRow[] = [
  ...['product.view','product.manage','event_template.view','event_template.manage','user.manage','db.manage','instance.create','instance.approve_qa','instance.execute_qa','instance.verify_qa','instance.approve_live','instance.execute_live','instance.verify_live'].map((strPermission) => ({ nRoleId: 1, strPermission })),
  ...['instance.execute_qa','instance.execute_live'].map((strPermission) => ({ nRoleId: 2, strPermission })),
  ...['product.view','event_template.view','instance.create','instance.approve_qa','instance.verify_qa','instance.approve_live','instance.verify_live'].map((strPermission) => ({ nRoleId: 3, strPermission })),
  ...['product.view','event_template.view','instance.create'].map((strPermission) => ({ nRoleId: 4, strPermission })),
];
