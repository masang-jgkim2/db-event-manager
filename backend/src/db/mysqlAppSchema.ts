/**
 * DQPM 메타 MySQL — 정규화 스키마 (docs/dqpm_meta_relational_schema.sql 과 동기).
 * 테이블명 `dqpm_` 접두사 없음. 동명 제품명 허용을 위해 `product.str_name` 에 UNIQUE 없음.
 */

/** CREATE TABLE … 구문에서 물리 테이블명 추출(부트 로그용) */
export const fnExtractMysqlDdlTableName = (strSql: string): string | null => {
  const m = strSql.match(/CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+`?(\w+)`?/i);
  return m?.[1] ?? null;
};

export const ARR_MYSQL_APP_DDL: string[] = [
...
COMMENT='products.json arrServices[]'`,
  ...
  str_status        VARCHAR(32)   NOT NULL DEFAULT 'active' COMMENT 'active|pending_approval|…',
  dt_created_at     DATETIME(6)   NOT NULL,
...
COMMENT='events.json 본문(세트 제외)'`,
  ...
  str_input_id        VARCHAR(64)   NOT NULL DEFAULT 'items' COMMENT 'IQueryTemplateItem.strInputId (arrInputs[0] 미러)',
  str_input_format    VARCHAR(64)   NOT NULL DEFAULT 'item_number' COMMENT 'IQueryTemplateItem.strInputFormat (arrInputs[0] 미러)',
  str_default_items   TEXT          NULL,
  json_arr_inputs     JSON          NULL COMMENT 'IQueryTemplateItem.arrInputs[] 전체 슬롯',
...
export const ARR_META_TABLE_NAMES: readonly string[] = [
  'product',
  'product_service',
  'users',
  'roles',
