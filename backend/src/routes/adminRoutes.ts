...
import { fnAdminOnly } from '../middleware/permissionMiddleware';
import { fnSaveSeedTest } from '../data/seedTest';

const router = Router();

// 현재 메모리 데이터를 테스트 초기화 데이터(seed_test.json)로 저장 — 관리자 전용
router.post('/save-test-seed', fnAuthMiddleware, fnAdminOnly, (_req: Request, res: Response) => {
  try {
    fnSaveSeedTest();
