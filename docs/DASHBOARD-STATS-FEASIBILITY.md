...
- **권한**: `GET /` 목록은 `dashboard.view`로도 호출 가능(통계용). `filter=all`이면 **전체 인스턴스**가 내려감 — 역할 설계 시 민감도 검토.
...
- **프론트**: `front/src/utils/instanceAggregateStats.ts` — 생성자/프로덕트/이벤트 템플릿별 집계, 영구 삭제 제외.
- **대시보드 UI**: `DashboardPage` 상단에 카드 **「이벤트 인스턴스 통계 (프로토타입)」** — 탭 3개 + 테이블(상위 50행, 페이지네이션).
- **데이터 로드**: `dashboard.view`만 있어도 인스턴스 목록을 요청하도록 로드 조건 확장.
- **API**: `GET /api/event-instances` 허용 권한에 `dashboard.view` 추가 (목록·통계용; 필터 `all` 시 전체).

---

## 7. 다음 작업(추가로 할 수 있는 것)
