...
> **브라우저(사람형) 테스트 통합 계획**: [E2E-PAGE-TEST-PLAN.md](./E2E-PAGE-TEST-PLAN.md)  
> 아래는 **역할·권한 요약**과 **Jest API 보조 테스트** 안내이다.
...
페이지·버튼 검증의 **공식 경로**는 Playwright이다. 통합 계획·페이지 매트릭스:

- **[E2E-PAGE-TEST-PLAN.md](./E2E-PAGE-TEST-PLAN.md)**
- 실행: `cd front && npm run test:e2e:smoke`

### Jest API (보조 — 403/200·순수 로직)

백엔드에서 **역할/권한에 따른 API 접근(200 vs 403)**을 검증한다. 브라우저 UI를 대체하지 않는다.
