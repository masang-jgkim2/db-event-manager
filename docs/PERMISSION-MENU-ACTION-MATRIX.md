...
| | `role.edit` | 역할 수정 | 커스텀 역할 정보·권한 수정 |
| | `role.delete` | 역할 삭제 | 역할 삭제(커스텀만) |
| | `role.edit_permissions` | 역할 권한 수정 | 시스템 역할 역할명·설명·권한 수정 |
...
| 수정 | 역할 수정 | 커스텀 역할 역할명·설명·권한(수정 버튼 → 모달) | `role.edit` |
| 삭제 | 역할 삭제 | 역할 삭제(커스텀 역할만, 삭제 버튼) | `role.delete` |
| 기타 | 시스템 역할 수정 | 시스템 역할 역할명·설명·권한(수정 버튼 → 모달, 코드 변경 불가) | `role.edit_permissions` |
