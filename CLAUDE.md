...
All target-DB execution funnels through `fnExecuteQueryWithText` in `services/queryExecutor.ts`. Connection selection goes through `fnResolveExecuteConnection(nProductId, strEnv, nDbConnectionId?, strServiceAbbr?)` — instance Step2 service scope with service-specific then common fallback; per-set `nDbConnectionId` + kind for multi-set templates. The legacy `fnFindActiveConnection` is non-deterministic and must not be used in the execute path.
...
### Query-template workflow (EventPage)
Three-state: `template_created → confirm_requested → dba_confirmed`. Transitions via `PATCH /api/events/:id/status` (`event_template.request_confirm` / `event_template.confirm`). **D1**: only `dba_confirmed` templates may create instances (`fnIsTemplateReadyForInstance`, QueryPage gate). **D2**: `PUT /api/events/:id` with query/set body changes on `dba_confirmed` → auto `confirm_requested` + re-approval log. During `confirm_requested`, query edits use `PUT /api/events/:id/query` only (`event_template.confirm`); general PUT blocks query fields. Legacy templates without `strStatus` normalize to `dba_confirmed`.

### Event-instance workflow (7 steps)
`event_created → qa_requested → qa_deployed → qa_verified → live_requested → live_deployed → live_verified`. Template confirm steps live on the template track only. Live-only deploy scope skips QA states. Re-request: `qa_verified→qa_requested`, `live_deployed|live_verified→live_requested`. Transition table in `OBJ_STATUS_TRANSITIONS_BASE`, gated by `arrDeployScope` via `fnGetTransitions`. Deploy-date guard: QA needs `dtQaDeployDate <= now` (if set), LIVE needs `dtLiveDeployDate ?? dtDeployDate <= now`; DEV is never executed directly. Failed executions keep state and append a failure row to `arrStatusLogs` with `bSuccess: false`, `strError`, `strConnectionSummary`. DBA instance query editing (`strGeneratedQuery` / per-set queries) only in `qa_requested` / `live_requested` (`my_dashboard.query_edit`). Legacy active statuses `confirm_requested`/`dba_confirmed` normalize to `event_created` on load (D7). See `.cursor/rules/domain-event-instance.mdc`.
